...
2. **Database:** Apply migration `supabase/migrations/20260408120000_profiles_payment_plan_months.sql` (Supabase SQL editor or `supabase db push` after link) so `profiles.payment_plan_months` exists and nulls are backfilled to **36**. Run `20260408140000_payment_plan_months_comment.sql` if present to refresh the column comment.
