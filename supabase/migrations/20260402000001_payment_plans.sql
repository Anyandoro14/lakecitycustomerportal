-- Migration: Add payment plan columns to contracts + contract_balances view

-- Add new columns to contracts (all with defaults so existing rows are unaffected)
ALTER TABLE contracts ADD COLUMN IF NOT EXISTS term_months INTEGER NOT NULL DEFAULT 36;
ALTER TABLE contracts ADD COLUMN IF NOT EXISTS plan_type TEXT NOT NULL DEFAULT 'standard';
ALTER TABLE contracts ADD COLUMN IF NOT EXISTS deposit_amount NUMERIC(12,2) DEFAULT 0;
ALTER TABLE contracts ADD COLUMN IF NOT EXISTS currency TEXT NOT NULL DEFAULT 'USD';
ALTER TABLE contracts ADD COLUMN IF NOT EXISTS is_vat_inclusive BOOLEAN;
ALTER TABLE contracts ADD COLUMN IF NOT EXISTS agreement_signed_seller BOOLEAN DEFAULT FALSE;
ALTER TABLE contracts ADD COLUMN IF NOT EXISTS agreement_signed_buyer BOOLEAN DEFAULT FALSE;
ALTER TABLE contracts ADD COLUMN IF NOT EXISTS agreement_file_url TEXT;

-- Composite index for fast dashboard lookups on payment_receipts
CREATE INDEX IF NOT EXISTS idx_payment_receipts_stand_tenant_qc
  ON payment_receipts(stand_number, tenant_id, qc_status);

-- Computed balance view: single query returns everything the dashboard needs
CREATE OR REPLACE VIEW contract_balances AS
SELECT
  c.id AS contract_id,
  c.tenant_id,
  c.customer_id,
  c.stand_number,
  c.total_price,
  c.deposit_amount,
  c.monthly_installment,
  c.term_months,
  c.payment_start_date,
  c.status,
  COALESCE(SUM(pr.amount) FILTER (WHERE pr.qc_status = 'approved'), 0) AS total_paid,
  c.total_price - COALESCE(SUM(pr.amount) FILTER (WHERE pr.qc_status = 'approved'), 0) AS current_balance,
  ROUND(
    COALESCE(SUM(pr.amount) FILTER (WHERE pr.qc_status = 'approved'), 0) / NULLIF(c.total_price, 0) * 100, 1
  ) AS progress_percentage,
  COUNT(*) FILTER (WHERE pr.qc_status = 'approved') AS payments_count,
  MAX(pr.payment_date) FILTER (WHERE pr.qc_status = 'approved') AS last_payment_date,
  MAX(pr.amount) FILTER (WHERE pr.qc_status = 'approved' AND pr.payment_date = (
    SELECT MAX(pr2.payment_date) FROM payment_receipts pr2
    WHERE pr2.stand_number = c.stand_number AND pr2.tenant_id = c.tenant_id AND pr2.qc_status = 'approved'
  )) AS last_payment_amount
FROM contracts c
LEFT JOIN payment_receipts pr ON pr.stand_number = c.stand_number AND pr.tenant_id = c.tenant_id
GROUP BY c.id;

-- Enable Realtime on payment_receipts and installments for live dashboard updates
ALTER PUBLICATION supabase_realtime ADD TABLE payment_receipts;
ALTER PUBLICATION supabase_realtime ADD TABLE installments;
