...
serve(async (req) => {
  ...
  try {
    const { sheetName, cellRange } = await req.json();
    
    if (!sheetName || !cellRange) {
      return new Response(
        JSON.stringify({ error: "sheetName and cellRange are required" }),
    ...
    console.log(`Clearing cell: ${fullRange}`);
    
    const updateUrl = `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${encodeURIComponent(fullRange)}?valueInputOption=USER_ENTERED`;
    ...
    if (!response.ok) {
      ...
      console.error(`Failed to clear cell: ${errorText}`);
      return new Response(
        JSON.stringify({ error: "Failed to clear cell", details: errorText }),
        { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    return new Response(
      JSON.stringify({ success: true, message: `Cleared ${fullRange}` }),
