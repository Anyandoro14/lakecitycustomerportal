...
serve(async (req) => {
  ...
  try {
    ...
    // Step 2: Read Collection Schedule - 36mo to find row indices for each stand
    const collectionRange = encodeURIComponent("Collection Schedule - 36mo!A:BH");
    const collectionUrl = `https://sheets.googleapis.com/v4/spreadsheets/${spreadsheetId}/values/${collectionRange}`;
    
    console.log('Fetching Collection Schedule - 36mo data...');
    ...
    if (!collectionResponse.ok) {
      ...
      throw new Error(`Failed to fetch Collection Schedule - 36mo: ${error}`);
    }
    
    const collectionData = await collectionResponse.json();
    const collectionRows = collectionData.values || [];
    
    console.log(`Collection Schedule - 36mo has ${collectionRows.length} rows`);
    ...
    // Payment columns in Collection Schedule - 36mo (based on screenshot):
    ...
    for (const standData of standsData) {
      ...
      if (!rowIndex) {
        results.push({
          ...
          paymentsWritten: 0,
          error: 'Stand not found in Collection Schedule - 36mo'
        });
        continue;
      }
      ...
      if (standData.phone) {
        updates.push({
          range: `Collection Schedule - 36mo!D${rowIndex}`,
          values: [[standData.phone]]
        });
      }
      
      // Write payments to Collection Schedule - 36mo in interleaved format:
      ...
      for (let p = 0; p < maxPayments; p++) {
        ...
        if (p < standData.payments.length) {
          ...
          updates.push({
            range: `Collection Schedule - 36mo!${amountColLetter}${rowIndex}`,
          ...
          updates.push({
            range: `Collection Schedule - 36mo!${dateColLetter}${rowIndex}`,
